...
This program creates a record of every network connection on a system and generates a log that contains the process name that initiated the connection. packetTrail supports forwarding these logs real time via syslog. This data has potential to assist SOC/IR teams in incident response or threat hunting.    
```
Log Sample:
